...
collect_results() {
    ...
    local clang_precision=`bc <<< "scale=2; $n_true_positive_clang/($n_true_positive_clang + $n_false_positive_clang) * 100"`
    local clang_recall=`bc <<< "scale=2; $n_true_positive_clang/$n_ground * 100"`
    local clog_precision=`bc <<< "scale=2; $n_true_positive_clog/($n_true_positive_clog + $n_false_positive_clog) * 100"`
    local clog_recall=`bc <<< "scale=2; $n_true_positive_clog/$n_ground * 100"`
    local clang_time=`cat $output_dir/clang.analysis.time`
    local clog_time=`cat $output_dir/clog.analysis.time`

    echo "$name & $clang_args & $n_ground & $n_true_positive_clang & $n_true_positive_clog & $clang_recall & $clog_recall & $n_false_positive_clang & $n_false_positive_clog & $clang_precision & $clog_precision & $clang_time & $clog_time \\tabularnewline" >> $file
}
